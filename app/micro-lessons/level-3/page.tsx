...
import { level3Lessons } from "./lessons.level3";
...
const TOPICS = [
  { name: "Health, safety and welfare in construction", order: 1 },
  { name: "Pricing construction work", order: 2 },
  { name: "Using machinery", order: 3 },
  { name: "Constructing cut roofing", order: 4 },
  { name: "Fixing stairs with turns", order: 5 },
  { name: "Manufacturing stairs with turns", order: 6 },
  { name: "Manufacturing curved joinery", order: 7 },
  { name: "Fitting doors, windows and furniture", order: 8 },
  { name: "Principles of maintenance and repair", order: 9 },
];

export default function MicroLessonsLevel3Page() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (nextUser) => {
      ...
      try {
        ...
        if (!hasPro && isMicroLessonLevelLockedForFreePlan("3")) {
          router.replace("/account");
        }
      } catch (error) {
  ...
  const lessons = useMemo(() => {
    const cleaned = level3Lessons.filter((lesson) => lesson.title.trim());
    if (!query.trim()) {
      return cleaned;
    }
  ...
  const grouped = useMemo(() => {
    const baseLessons = level3Lessons.filter((lesson) => lesson.title.trim());
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-5 pb-20 pt-32 sm:px-8">
        ...
        <header className="space-y-2">
          ...
          <p className="text-sm text-[var(--muted)] sm:text-base">
            Short revision notes built for Level 3 joinery theory.
          </p>
        </header>
